...
const omdbFindMovieByID = async (id) => {
...
const omdbFetchAllTheMovies = async (movie) => {
	...
	let all_movies = '';	// This variable will recolect all the movies
	let loading_movies = 'Las peliculas están cargando, espere un momento por favor...';
	movies_html.innerHTML = loading_movies;
	if (!results) {
		movies_html.innerHTML = 'No se encuentran resultados en su búsqueda';
	} else {
		for (let result of results) {
			console.log(result.imdbID);
			//omdbID(result.imdbID);
			const movie = await omdbFindMovieByID(result.imdbID)
			if (movie) {
				all_movies += movie;
			}
...
document.querySelector('#movie-name').addEventListener('submit', (e) => {
	...
	omdbFetchAllTheMovies(UsableClass.textOMDB(name));
	e.preventDefault();
});
